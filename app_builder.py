...
<head>
    ...
    <script type="module">
        ...
        async function cargarDatosJSON() {
            try {
                ...
                estadosArr.forEach(est => {
                    const checked = (est === "CONSTRUIDO") ? "" : "checked";
                    drop.innerHTML += `
                        <label class="flex items-center gap-2 p-1.5 hover:bg-blue-50 cursor-pointer rounded">
                            <input type="checkbox" value="${est}" class="chk-estado w-4 h-4 text-personal-cyan rounded border-gray-300" ${checked}> 
                            <span class="truncate font-semibold text-gray-700">${est}</span>
                        </label>
                    `;
                });
        ...
        window.aplicarFiltros = async () => {
            ...
            if (!subregion && !pd && !calle && !altura && !estRelevo && !auditor) {
                alert("Por favor, ingrese al menos un filtro de texto (PD, Calle, etc.) para no saturar la búsqueda.");
                btnBuscar.innerText = "BUSCAR DIRECCIONES"; return;
            }

            if (estadosSeleccionados.length === 0) {
                alert("Debe seleccionar al menos un Estado Constructivo para buscar.");
                btnBuscar.innerText = "BUSCAR DIRECCIONES"; return;
            }
            ...
            for (const edif of datosPagina) {
                ...
                card.innerHTML = `
                    ...
                    <div class="mt-4 bg-white/50 p-3 rounded-lg border border-gray-200 shadow-inner">
                        ...
                        <div class="space-y-3">
                            <select id="accion-${edif.ACTIVO}" onchange="manejarAccionesDinamicas('${edif.ACTIVO}')" class="w-full p-2 border border-gray-300 rounded text-sm bg-white focus:ring-1 focus:ring-personal-cyan outline-none font-medium text-personal-navy">${opcionesSelect}</select>
        ...
        window.manejarAccionesDinamicas = (activo) => {
            ...
            if (accion === "CORREGIR ALTURA") {
                ...
                `;
            } else if (accion === "UNIFICAR DIRECCIONES") {
                container.innerHTML = `
                    <label class="block text-xs font-bold text-personal-navy mb-1">Buscar dirección a unificar (*)</label>
                    <div id="unificar-lista-${activo}" class="flex flex-col gap-2 mb-2"></div>
                    <button type="button" onclick="agregarBuscadorUnificar('${activo}')" class="text-xs bg-white border-2 border-personal-cyan text-personal-cyan font-bold py-1.5 px-3 rounded w-max shadow-sm">+ Sumar dirección</button>
                `;
                agregarBuscadorUnificar(activo);
            } else if (accion === "NO ES EDIFICIO") {
                container.innerHTML = `
                    <label class="block text-xs font-bold text-personal-navy mb-1">Tipo de inmueble (*)</label>
                    <select id="tipo-inmueble-${activo}" class="w-full p-2 border rounded text-sm outline-none bg-white" onchange="verificarOtroInmueble('${activo}')">
                        <option value="">Seleccione...</option>
                        <option value="Casa">Casa</option>
                        <option value="PH menor a 6 UF">PH menor a 6 UF</option>
                ...
                container.innerHTML = `<p class="text-[11px] font-bold text-amber-600 uppercase">⚠️ Aclare con qué empresa está armado en la observación.</p>`;
            } else if (accion === "NO EXISTE ALTURA") {
                container.innerHTML = `<p class="text-[11px] font-bold text-amber-600 uppercase">⚠️ Explique el motivo (Ej: Fue demolido, numeración saltada) en la observación.</p>`;
            } else {
        ...
        window.agregarBuscadorUnificar = (activo) => {
            const idRow = Date.now(); const lista = document.getElementById(`unificar-lista-${activo}`);
            const div = document.createElement('div'); div.className = "p-2 bg-white border border-gray-300 rounded shadow-sm flex flex-col gap-2"; div.id = `row-unif-${idRow}`;
            div.innerHTML = `
                <div class="flex gap-2">
                    <input type="text" id="unif-calle-${idRow}" placeholder="Calle parcial" class="w-2/3 p-2 border rounded text-xs uppercase bg-gray-50">
                    <input type="number" id="unif-alt-${idRow}" placeholder="Altura" class="w-1/3 p-2 border rounded text-xs bg-gray-50">
                </div>
                <div class="flex justify-between items-center">
                    <button type="button" onclick="validarDireccionUnificar('${idRow}')" class="bg-blue-100 text-blue-800 px-3 py-1 rounded text-xs font-bold border border-blue-200 hover:bg-blue-200 transition">Validar</button>
                    <span id="unif-status-${idRow}" class="text-[10px] font-bold text-gray-400" data-validado="false">Pendiente</span>
                    <button type="button" onclick="document.getElementById('row-unif-${idRow}').remove()" class="text-red-500 font-extrabold text-xs px-2">X</button>
                </div>
            `;
            lista.appendChild(div);
        };

        window.validarDireccionUnificar = (idRow) => {
            const inputCalle = document.getElementById(`unif-calle-${idRow}`);
            const calleTexto = inputCalle.value.trim().toUpperCase();
            const altura = document.getElementById(`unif-alt-${idRow}`).value.trim();
            const statusSpan = document.getElementById(`unif-status-${idRow}`);

            if(!calleTexto || !altura) {
                statusSpan.innerText = "⚠️ Faltan datos"; statusSpan.className = "text-[10px] font-bold text-red-500"; statusSpan.dataset.validado = "false"; return;
            }

            const encontrado = maestroDatos.find(edif => {
                const matchPpal = (edif.CALLE || "").toUpperCase().includes(calleTexto) && String(edif.ALTURA) === altura;
                const matchSec = edif.DIRECCIONES_SECUNDARIAS.some(sec => (sec.CALLE || "").toUpperCase().includes(calleTexto) && String(sec.ALTURA) === altura);
                return matchPpal || matchSec;
            });

            if (encontrado) {
                inputCalle.value = encontrado.CALLE; 
                statusSpan.innerText = `✅ Validado (ID: ${encontrado.ACTIVO})`;
                statusSpan.className = "text-[10px] font-bold text-green-600";
                statusSpan.dataset.validado = "true"; statusSpan.dataset.activo = encontrado.ACTIVO; statusSpan.dataset.str = `${encontrado.CALLE} ${altura}`;
            } else {
                statusSpan.innerText = "❌ No existe activo"; statusSpan.className = "text-[10px] font-bold text-red-500"; statusSpan.dataset.validado = "false";
            }
        };

        window.verificarOtroInmueble = (activo) => {
            const val = document.getElementById(`tipo-inmueble-${activo}`).value;
            const leyenda = document.getElementById(`leyenda-inmueble-${activo}`);
            if(val === "Otro") leyenda.classList.remove('hidden'); else leyenda.classList.add('hidden');
        };
        ...
        window.agregarAccionLista = (activo) => {
            ...
            if (accion === "CORREGIR ALTURA") {
                ...
                let extrasStr = ""; const chkOtras = document.getElementById(`chk-otras-alt-${activo}`);
                ...
                const statusSpans = document.getElementById(`unificar-lista-${activo}`).querySelectorAll('span[id^="unif-status-"]');
                if(statusSpans.length === 0) { alert("Debe sumar al menos una dirección para unificar."); return; }
                
                let unificadas = []; let hayInvalidos = false;
                statusSpans.forEach(span => {
                    if (span.dataset.validado === "true") unificadas.push(`[${span.dataset.activo}] ${span.dataset.str}`);
                    else hayInvalidos = true;
                });
                if (hayInvalidos || unificadas.length === 0) { alert("Hay direcciones sin validar (❌). Valídelas o bórrelas antes de guardar."); return; }
                obsGeneral = `[Unificar con: ${unificadas.join(', ')}] ${obsGeneral ? '- Obs: ' + obsGeneral : ''}`;
            
            } else if (accion === "NO ES EDIFICIO") {
                const tipoInm = document.getElementById(`tipo-inmueble-${activo}`).value;
            ...
            if (!obsGeneral && ["IMPOSIBLE CONSTRUIR", "COMPETENCIA", "NO EXISTE ALTURA"].includes(accion)) {
                alert("Para esta acción, la observación es obligatoria indicando los motivos."); return;
            }

            window.accionesPendientes[activo].push({ accion: accion, observacion: obsGeneral || "Sin observación adicional" });
            
            document.getElementById(`accion-${activo}`).value = ''; document.getElementById(`obs-${activo}`).value = '';
            document.getElementById(`form-dinamico-${activo}`).innerHTML = ''; document.getElementById(`form-dinamico-${activo}`).classList.add('hidden');
            renderAccionesLocales(activo);
        };

        window.quitarAccionLocal = (activo, index) => { window.accionesPendientes[activo].splice(index, 1); renderAccionesLocales(activo); };
        ...
        window.renderAccionesLocales = (activo) => {
            ...
            container.innerHTML = pend.map((p, i) => `
                <div class="flex justify-between items-center bg-blue-50 border border-blue-200 p-2 rounded text-sm shadow-sm">
                    <div class="flex-1"><span class="font-bold text-personal-navy text-[11px] uppercase">${p.accion}</span><br><span class="text-[10px] text-gray-600">${p.observacion}</span></div>
            ...
            if (pend.length > 0) panelEnvio.classList.remove('hidden'); else panelEnvio.classList.add('hidden');
        };
        ...
        window.enviarRelevoFinal = async (activo) => {
            ...
            try {
                ...
                if(result.status === "success") {
                    ...
                    let nuevoHistorial = [];
                    if (mapaRelevos[activo] && mapaRelevos[activo].historial) nuevoHistorial = mapaRelevos[activo].historial;
                    else if (mapaRelevos[activo]) nuevoHistorial = [mapaRelevos[activo]]; 
        ...
        <div class="bg-white shadow-sm z-10 p-4 border-b">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    <div class="relative">
                        <button type="button" onclick="document.getElementById('dropdown-estados').classList.toggle('hidden')" class="w-full mt-1 p-2 border rounded bg-gray-50 text-sm text-left flex justify-between items-center font-bold text-personal-navy">
                            <span class="truncate">Filtrar Estados...</span> <span>▼</span>
                        </button>
                        <div id="dropdown-estados" class="hidden absolute z-30 w-full bg-white border border-gray-300 shadow-xl max-h-48 overflow-y-auto p-2 mt-1 rounded-lg text-xs"></div>
                    </div>
                </div>

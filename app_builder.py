...
def generar_html():
    print("4. Generando index.html con paginación, ordenamiento y cámara...")
...
<head>
    ...
    <script type="module">
        ...
        window.fotosActivos = {}; // Guarda { "ED-123": [base64, base64...], ... }
        ...
        window.aplicarFiltros = async () => {
            ...
            // Ordenamiento Alfabético (Calle -> Altura)
            resultados.sort((a, b) => {
            ...
            if (resultados.length > maxResultados) {
                ...
                document.getElementById('resultados-count').innerHTML = `<span class="text-amber-600 font-bold">Límite alcanzado:</span> Mostrando ${maxResultados} activos ordenados por ruta.`;
            } else {
        ...
        window.renderPagina = () => {
            ...
            for (const edif of datosPagina) {
                ...
                if (!fotosActivos[edif.ACTIVO]) fotosActivos[edif.ACTIVO] = []; // Inicializar memoria fotos
                ...
                if (estaRelevado) {
                    ...
                    card.innerHTML = `
                        ...
                        <div class="mt-4 space-y-3 bg-white/50 p-3 rounded-lg border border-gray-100">
                            <select id="accion-${edif.ACTIVO}" class="w-full p-2.5 border border-gray-300 rounded-lg text-sm bg-white focus:ring-2 focus:ring-personal-cyan outline-none font-medium">${opcionesSelect}</select>
                            ...
                            <div class="bg-gray-50 border border-gray-200 p-3 rounded-lg">
                                <div class="flex justify-between items-center mb-2">
                                    ...
                                    <div class="flex gap-2">
                                        <button onclick="document.getElementById('cam-${edif.ACTIVO}').click()" class="bg-blue-100 text-blue-700 px-3 py-1.5 rounded-md text-xs font-bold shadow-sm border border-blue-200 flex items-center gap-1 hover:bg-blue-200">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z"></path></svg> Cámara
                                        </button>
                                        <button onclick="document.getElementById('gal-${edif.ACTIVO}').click()" class="bg-gray-200 text-gray-700 px-3 py-1.5 rounded-md text-xs font-bold shadow-sm border border-gray-300 flex items-center gap-1 hover:bg-gray-300">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-8l-4-4m0 0L8 8m4-4v12"></path></svg> Galería
                                        </button>
                                    </div>
                ...
                // Si ya había fotos previas en memoria para este activo (por ej. si cambió de página), las redibuja
            ...
            window.scrollTo(0, 0); // Sube arriba de todo al cambiar de página
        };
        ...
        // --- LÓGICA DE FOTOS ACUMULATIVAS ---
        window.agregarFotos = async (activo, files) => {
            if (!files || files.length === 0) return;
            let currentFotos = window.fotosActivos[activo] || [];
            
            for (let i = 0; i < files.length; i++) {
                if (currentFotos.length >= 15) { alert("Has alcanzado el límite máximo de 15 fotos."); break; }
        ...
        window.eliminarFoto = (activo, index) => {
            window.fotosActivos[activo].splice(index, 1);
            dibujarMiniaturas(activo);
        };
        ...
        window.dibujarMiniaturas = (activo) => {
            ...
            if(!container) return; // Si la tarjeta no está en el DOM actual
            
            container.innerHTML = '';
            fotos.forEach((b64, index) => {
                container.innerHTML += `
                    <div class="relative w-12 h-12 shadow-sm rounded border border-gray-300">
                        <img src="${b64}" class="object-cover w-full h-full rounded">
                        <button onclick="eliminarFoto('${activo}', ${index})" class="absolute -top-1.5 -right-1.5 bg-red-500 text-white rounded-full w-5 h-5 text-[10px] flex items-center justify-center font-extrabold shadow-md">X</button>
                    </div>`;
            });
        ...
        window.enviarRelevo = async (activo) => {
            ...
            try {
                ...
                if(result.status === "success") {
                    ...
                    window.fotosActivos[activo] = []; // Limpiar fotos de la memoria RAM
                    aplicarFiltros(); 
                } else {
        ...
        <div class="bg-white shadow-sm z-10 p-4 border-b">
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    <select id="filt-subregion" class="w-full mt-1 p-2 border rounded bg-gray-50 text-sm"><option value="">Todas</option><option value="CAPITAL NORTE">Capital Norte</option><option value="CAPITAL SUR">Capital Sur</option></select>
                </div>
            ...
            <div class="grid grid-cols-2 gap-3 mb-3">
                <div>
                    ...
                    <select id="filt-estado-const" class="w-full mt-1 p-2 border rounded bg-gray-50 text-sm"><option value="">Todos</option><option value="CONSTRUIDO">Construido</option><option value="A CONSTRUIR">A Construir</option><option value="SIN ESTADO">Sin Estado</option></select>
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-gray-500 uppercase">Estado Relevo</label>
                    <select id="filt-estado-relevo" class="w-full mt-1 p-2 border rounded bg-gray-50 text-sm font-bold text-personal-navy"><option value="">Todos</option><option value="PENDIENTE" selected>Falta Relevar</option><option value="RELEVADO">Relevado</option></select>
                </div>
            </div>

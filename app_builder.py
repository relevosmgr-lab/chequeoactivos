...
def generar_html():
    print("4. Generando index.html con todos los formularios dinámicos...")
...
<head>
    ...
    <script type="module">
        ...
        // --- MANEJO DE FORMULARIOS DINÁMICOS Y ALERTAS ---
        ...
        // -> Lógica UNIFICAR DIRECCIONES
        window.agregarBuscadorUnificar = (activo) => {
            const idRow = Date.now();
            const lista = document.getElementById(`unificar-lista-${activo}`);
            const div = document.createElement('div');
            div.className = "p-2 bg-white border border-gray-300 rounded shadow-sm flex flex-col gap-2";
            div.id = `row-unif-${idRow}`;
            div.innerHTML = `
                <div class="flex gap-2">
        ...
        window.validarDireccionUnificar = (idRow) => {
            ...
            if(!calleTexto || !altura) {
                statusSpan.innerText = "⚠️ Faltan datos";
                statusSpan.className = "text-[10px] font-bold text-red-500";
                statusSpan.dataset.validado = "false"; return;
            }
            ...
            if (encontrado) {
                inputCalle.value = encontrado.CALLE; // Autocompleta la calle exacta encontrada
                statusSpan.innerText = `✅ Validado (ID: ${encontrado.ACTIVO})`;
                statusSpan.className = "text-[10px] font-bold text-green-600";
                statusSpan.dataset.validado = "true";
                statusSpan.dataset.activo = encontrado.ACTIVO;
                statusSpan.dataset.str = `${encontrado.CALLE} ${altura}`;
            } else {
                statusSpan.innerText = "❌ No existe activo";
                statusSpan.className = "text-[10px] font-bold text-red-500";
                statusSpan.dataset.validado = "false";
            }
        };

        // -> Lógica NO ES EDIFICIO
        window.verificarOtroInmueble = (activo) => {
            ...
            if(val === "Otro") leyenda.classList.remove('hidden');
            else leyenda.classList.add('hidden');
        };
        ...
        window.agregarAccionLista = (activo) => {
            ...
            // Procesar y empaquetar datos dinámicos según la acción
            if (accion === "CORREGIR ALTURA") {
                ...
                if (hayInvalidos || unificadas.length === 0) { alert("Hay direcciones sin validar (❌). Por favor valídelas o elimínelas con la X antes de guardar."); return; }
            ...
            // Validaciones de Observación obligatoria
